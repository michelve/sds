...
async function initialize() {
  ...
  fs.mkdirSync(`${WRITE_DIR}/css`, { recursive: true });
  fs.mkdirSync(`${WRITE_DIR}/scss`, { recursive: true });
  fs.mkdirSync(`${WRITE_DIR}/js`, { recursive: true });
  fs.mkdirSync(`${WRITE_DIR}/json`, { recursive: true });
  fs.mkdirSync(`${WRITE_DIR}/react-native`, { recursive: true });
  fs.mkdirSync(`${WRITE_DIR}/ios`, { recursive: true });
  fs.mkdirSync(`${WRITE_DIR}/android`, { recursive: true });

  // CSS Output (existing format)
  fs.writeFileSync(
    `${WRITE_DIR}/css/tokens.css`,
  ...
  const scssContent = [...themeCSS, ...stylesCSS]
    .join("\n")
    .replace(/--sds-/g, '$sds-')
    .replace(/:root\s*{/g, '// SCSS Variables')
    .replace(/}/g, '')
    .replace(/;/g, ' !default;');
  fs.writeFileSync(`${WRITE_DIR}/scss/tokens.scss`, scssContent);

  // JavaScript/TypeScript Output
  const jsTokens = extractTokensAsJS(processed);
  fs.writeFileSync(
    `${WRITE_DIR}/js/tokens.js`,
    `export const tokens = ${JSON.stringify(jsTokens, null, 2)};`
  );

  // JSON Output (flat structure)
  const flatTokens = extractTokensAsFlat(processed);
  fs.writeFileSync(
    `${WRITE_DIR}/json/tokens.json`,
    JSON.stringify(flatTokens, null, 2)
  );

  // React Native Output
  const rnTokens = extractTokensForReactNative(processed);
  fs.writeFileSync(
    `${WRITE_DIR}/react-native/tokens.js`,
    `export const tokens = ${JSON.stringify(rnTokens, null, 2)};`
  );

  // iOS Output (Objective-C header)
  const iosHeader = generateIOSHeader(processed);
  fs.writeFileSync(`${WRITE_DIR}/ios/tokens.h`, iosHeader);

  // Android Output (colors.xml)
...
// Helper functions for multi-platform output

function extractTokensAsJS(processed) {
  const tokens = {};
  for (const key in processed) {
    const collection = processed[key];
    if (collection.definitions) {
      for (const mode in collection.definitions) {
        const modeTokens = collection.definitions[mode];
        modeTokens.forEach(token => {
          const tokenKey = token.propertyName.replace(/^--sds-/, '').replace(/-/g, '.');
          tokens[tokenKey] = token.value;
        });
      }
    }
  }
...
function extractTokensAsFlat(processed) {
  const tokens = {};
  for (const key in processed) {
    const collection = processed[key];
    if (collection.definitions) {
      for (const mode in collection.definitions) {
        const modeTokens = collection.definitions[mode];
        modeTokens.forEach(token => {
          tokens[token.propertyName.replace(/^--/, '')] = token.value;
        });
      }
    }
  }
...
function extractTokensForReactNative(processed) {
  const tokens = {};
  for (const key in processed) {
    const collection = processed[key];
    if (collection.definitions) {
      for (const mode in collection.definitions) {
        ...
        modeTokens.forEach(token => {
          ...
          const tokenKey = token.propertyName
            .replace(/^--sds-/, '')
...
function generateIOSHeader(processed) {
  let header = '#import <UIKit/UIKit.h>\n\n@interface SDSTokens : NSObject\n\n';
  
  for (const key in processed) {
    const collection = processed[key];
    if (collection.definitions && collection.settings?.prefix === 'color') {
      for (const mode in collection.definitions) {
        const modeTokens = collection.definitions[mode];
        modeTokens.forEach(token => {
          if (token.value.startsWith('#')) {
            const methodName = token.propertyName
              .replace(/^--sds-/, '')
              .replace(/-([a-z])/g, (match, letter) => letter.toUpperCase());
            header += `+ (UIColor *)${methodName};\n`;
          }
        });
      }
    }
...
function generateAndroidXml(processed) {
  ...
  for (const key in processed) {
    ...
    if (collection.definitions && collection.settings?.prefix === 'color') {
      for (const mode in collection.definitions) {
        ...
        modeTokens.forEach(token => {
          if (token.value.startsWith('#')) {
            const colorName = token.propertyName
              .replace(/^--sds-/, '')
              .replace(/-/g, '_');
            xml += `    <color name="${colorName}">${token.value}</color>\n`;
          }
        });
      }
    }
